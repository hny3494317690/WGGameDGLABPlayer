...
class WGGameDGLABPlayer(object):
    def __init__(self):
        ...
        logger.info("WGGameDGLABPlayer模组初始化完成")
...
# 创建模组实例
g_WGGameDGLABPlayer = WGGameDGLABPlayer()

# 注册事件处理
def onPlayerDamaged(attackerID: int, damage: int):
    g_WGGameDGLABPlayer.onPlayerDamaged(attackerID, damage)
...
def init():
    try:
        ...
        logger.info("WGGameDGLABPlayer模组已初始化")
    except Exception as e:
...
def fini():
    try:
        ...
        logger.info("WGGameDGLABPlayer模组已关闭")
    except Exception as e:
